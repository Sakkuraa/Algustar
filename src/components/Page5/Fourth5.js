import ReactDOM from "react-dom";
import { NavLink } from "react-router-dom";
import React, { Component } from "react";

export default class Fourth5 extends Component {
  render() {
    return (
      <div className="main-middle2 main-top clearfix">
        <article>
          <ul>
            <br />
            <NavLink to="/First5">Аан Суорун</NavLink>
            <br />
            <br />
            <NavLink to="/Second5">Төрүт Суорун</NavLink>
            <br />
            <br />
            <NavLink to="/Third5">Түстүр Суорун</NavLink>
            <br />
            <br />
            <NavLink to="/Fourth5">Куулаттарар Суорун</NavLink>
            <br />
            <br />
            <NavLink to="/Fifth5">Үөрэх Суорун</NavLink>
            <br />
            <br />
            <NavLink to="/Sixth5">Уһуйар Суорун</NavLink>
            <br />
            <br />
            <NavLink to="/Seventh5">Идэ Суорун</NavLink>
            <br />
            <br />
          </ul>
        </article>
        <article>
          <br />
          <br />
          <h2 align="center">Куулаттарыы алгыһа</h2>
          <br />
          <p>
            Куулаттарар Суорун, <br />
            Бэттэх көрөн <br />
            Мичик гын! <br />
            Сүлгүр! <br />
            <br />
            Куулаттарыым холбостун,
            <br />
            Төрүттэниим түмүлүннүн.
            <br />
            <br />
            Испэр иччилэниим, <br />
            Таспар дьалыҥнаныым.
            <br />
            <br />
            Томторҕом арылыннын, <br />
            Түмүгүм сөлүннүн, <br />
            <br />
            Ытык дьонум <br />
            Ыйаахтаах тыллара <br />
            Ыксаластын.
            <br />
            <br />
            Төрүт дьонум <br />
            Төрүт тыллара <br />
            Төбөбөр түстүн. <br />
            <br />
            Ааттаммат аймахтарым
            <br />
            Харахтарын хатаатыннар.
            <br />
            <br />
            Сурахтаммат урууларым <br />
            Санааларын уурдуннар. <br />
            <br />
            Этим-сииним <br />
            Илбистэннин. <br />
            <br />
            Киирэрим киирдин,
            <br />
            Таарыйарым таарыйдын. <br />
            Дуом!
            <br />
            <br />
          </p>
        </article>
        <article>
          <br />
          <br />
          <h2 align="center">Молитва о жизненных силах</h2>
          <br />
          <p>
            Небесный Царь Выбора,
            <br />
            Повернись ко мне,
            <br />
            Улыбнись благосклонно!
            <br />
            Позволь обратиться!
            <br />
            <br />
            Пусть накапливается энергия
            <br />
            Для определения жизненного пути.
            <br />
            <br />
            Пусть в меня вселиться дух
            <br />
            И буду я в большом волнении.
            <br />
            <br />
            Пусть мозг откроется,
            <br />
            Развяжутся узлы сознания.
            <br />
            <br />
            Вспомню
            <br />
            Полезные советы
            <br />
            Уважаемых людей.
            <br />
            <br />
            Всплывут
            <br />
            Главные советы
            <br />
            Предков.
            <br />
            <br />
            Пусть близкие в этот миг
            <br />
            Следят за мной.
            <br />
            <br />
            Пусть родные
            <br />
            Думают обо мне.
            <br />
            <br />
            Пусть тело в это время
            <br />
            Хранит возбуждение.
            <br />
            <br />
            Пусть накапливается во мне
            <br />
            Возбуждающая жизненная энергия.
            <br />
            Да будет так!
            <br />
            <br />
          </p>
        </article>
      </div>
    );
  }
}
